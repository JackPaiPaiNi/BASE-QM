	...
	styleUI : {
		...
		Bootstrap : {
			...
			base : {
				...
				pagerTable : "table",
				toppagerBox : "",
				pgInput : "form-control input-sm",
				pgSelectBox : "form-control input-sm",
				pgButtonBox : "",
			...
			colmenu : {
				menu_widget : '',
				input_checkbox : "",
				filter_select: "form-control input-sm",
				filter_input : "form-control input-sm",
				...
				if( $(ind).hasClass('jqgrow') ) {
					if(usedata) {
						...
						$('td[role="gridcell"]',ind).each( function(i) {
							...
							if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && nm !== 'btn') {
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$(htbl).width(1);
				$(htbl).css("height",hth+"px");/*修改锁定列错位*/
		...
		return this.each(function(){
			...
			function fillData(rowid,obj,fmid){
				...
				$('td[role="gridcell"]',tre).each( function(i) {
					nm = cm[i].name;
					// hidden fields are included in the form
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && nm !== 'btn' && cm[i].editable===true) {
						if(nm === obj.p.ExpandColumn && obj.p.treeGrid === true) {
							tmp = $(this).text();
						} else {
							try {
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && nm !== 'btn' && viewfld) {
						if(ind === false) {
							tmp = "";
						} else {
							if(nm === obj.p.ExpandColumn && obj.p.treeGrid === true) {
				...
				if (iCol >= 0) {
					...
					if (skip === 0) {
						if (o.useColSpanStyle) {
							...
							// 王歌添加多行表头
							// $th.attr("rowspan", "2");
							var rowspanlength = $th.parent().siblings("tr").length;
							rowspanlength = rowspanlength == 0 ? 2 : rowspanlength + 1;
							$th.attr("rowspan",rowspanlength);
						} else {
...
//Editing
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && nm !== 'btn') {
						...
						if(cm[i].editable===true) {
							...
							else { $(this).addClass("nopadding").append(elc); /*去除编辑状态td的padding*/}
							$.jgrid.bindEv.call($t, elc, opt);
							//Again IE
	...
	saveRow : function(rowid, successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		var success = false, nm, tmp={}, tmp2={}, tmp3= {}, editable, fr, cv, ind, nullIfEmpty=false, skyErr = [],
		...
		if (editable==="1") {
			...
			$('td[role="gridcell"]',ind).each(function(i) {
				...
				nm = cm.name;
				elem = "";
				if ( nm !== 'cb' && nm !== 'subgrid' && cm.editable===true && nm !== 'rn' && nm !== 'btn' && !$(this).hasClass('not-editable-cell')) {
					switch (cm.edittype) {
						case "checkbox":
							var cbv = ["Yes","No"];
							if(cm.editoptions ) {
					...
					if(cv[0] === false) {
						/*-----------------------------------修改编辑表错误信息单个返回为整行返回--------注掉开始------------------------------*/
						/*index = i;
						return false;*/
						/*-----------------------------------修改编辑表错误信息单个返回为整行返回--------注掉结束------------------------------*/
						/*-----------------------------------修改编辑表错误信息单个返回为整行返回--------添加开始------------------------------*/
						var tmpErr = {index : i, 
								elem : elem,
								msg : cv[1]};
						skyErr.push(tmpErr);
						elem.addClass("has-error");
						/*-----------------------------------修改编辑表错误信息单个返回为整行返回--------添加结束------------------------------*/
					}
			...
			/*-----------------------------------修改编辑表错误信息单个返回为整行返回--------添加开始------------------------------*/
			if(skyErr.length > 0){
				index = skyErr[0].index;
				var msg = "";
				$.each(skyErr, function(idx, err){
					msg += err.msg + "；";
				});
				try {
					if($.isFunction(o.errorfunc) ) {
						o.errorfunc.call($t, rowid, ind.rowIndex, msg);
					}else{
						var tr = $($t).jqGrid('getGridRowById', rowid), 
						positions = $.jgrid.findPos(tr);
						$.jgrid.info_dialog(errors.errcap,"第" + ind.rowIndex + "行校验不通过：" + msg,edit.bClose,{
							left:positions[0],
			...
			/*-----------------------------------修改编辑表错误信息单个返回为整行返回--------注掉结束------------------------------*/
			/*在编辑是去除了padding，这里还原*/
			$(ind).find(".nopadding").removeClass("nopadding");
	...
	setTreeNode : function(i, len){
		return this.each(function(){
			...
			while(i<len) {
				...
				if(ldat[isLeaf] === "true" || ldat[isLeaf] === true || ldat[isLeaf] === "1" || ldat[isLeaf] === 1) {
				...
				ldat[expanded] = ((ldat[expanded] === "true" || ldat[expanded] === true || ldat[expanded] === "1" || ldat[expanded] === 1) ? true : false) && (ldat[loaded] || ldat[loaded] === undefined);
